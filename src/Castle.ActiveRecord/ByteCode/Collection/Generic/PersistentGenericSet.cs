...
namespace Castle.ActiveRecord.ByteCode.Collection.Generic
{
    ...
    class PersistentGenericSet<T> : NHibernate.Collection.Generic.PersistentGenericSet<T>
    {
        public PersistentGenericSet() { }

		public PersistentGenericSet(ISessionImplementor session) : base(session) { }

        public PersistentGenericSet(ISessionImplementor session, ISet<T> original) : base(session, original) { }
