...
namespace Castle.ActiveRecord.ByteCode.Collection.Generic
{
    ...
    class PersistentGenericBag<T> : NHibernate.Collection.Generic.PersistentGenericBag<T>
    {
        public PersistentGenericBag() { }

		public PersistentGenericBag(ISessionImplementor session) : base(session) { }

        public PersistentGenericBag(ISessionImplementor session, ICollection<T> coll) : base(session, coll) { }
