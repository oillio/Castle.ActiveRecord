...
namespace Castle.ActiveRecord.ByteCode.Collection.Generic
{
    ...
    class PersistentIdentifierBag<T> : NHibernate.Collection.Generic.PersistentIdentifierBag<T>
    {
        public PersistentIdentifierBag() { }

		public PersistentIdentifierBag(ISessionImplementor session) : base(session) { }

        public PersistentIdentifierBag(ISessionImplementor session, ICollection<T> coll) : base(session, coll) { }
