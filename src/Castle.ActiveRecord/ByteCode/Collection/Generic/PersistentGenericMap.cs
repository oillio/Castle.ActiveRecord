...
namespace Castle.ActiveRecord.ByteCode.Collection.Generic
{
    ...
    class PersistentGenericMap<TKey, TValue> : NHibernate.Collection.Generic.PersistentGenericMap<TKey, TValue>
    {
        public PersistentGenericMap() { }

		public PersistentGenericMap(ISessionImplementor session) : base(session) { }

        public PersistentGenericMap(ISessionImplementor session, IDictionary<TKey, TValue> map) : base(session, map) { }
